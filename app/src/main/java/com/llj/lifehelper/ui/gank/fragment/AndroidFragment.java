 ...
 * </pre>
 */
public class AndroidFragment extends BaseFragment implements GankContract.IGankView, SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {
	...
	protected void initView() {
		...
		rvAndroid.addItemDecoration(new DividerItemDecoration(getActivity(), DividerItemDecoration.VERTICAL));
		gankPresenter = new GankPresenter();
		gankPresenter.attachView(this);
		gankPresenter.requestGankResource("Android", PAGE_SIZE, 1);
	}
	...
	private void initAdapter() {
		adapter = new GankAdapter(gankList);
		adapter.setOnLoadMoreListener(this, rvAndroid);
		adapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
			@Override
			public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
				H5Activity.openActivity(getActivity(), gankList.get(position).getUrl());
			}
		});
	...
	public void onRefresh() {
		...
		gankPresenter.requestRefreshResource("Android", PAGE_SIZE, 1);
	}
	...
	public void onLoadMoreRequested() {
		...
		if (adapter.getData().size() < PAGE_SIZE) {
			...
			page++;
			gankPresenter.requestLoadMore("Android", PAGE_SIZE, page);
		}
	}
}
